module.exports = function(app){
    ...
    app.post('/api/add', function(req,res){
        var newTerm = {"term":req.body.term, "definition":req.body.definition};
        TermModel.findOne({'term': {'$regex' : '^' + req.body.term, '$options' : 'i'}}, function (err, found_term){
            if(err) return next(err);
                if (found_term) {
                    res.json({"result":"Term already in list"});            
                } else {
                    new TermModel(newTerm).save(function(err){
                    res.json({"result":"Term added"});  
                    });       
                };
        });
    }); 
    ...
    app.post('/api/update', function(req,res){       
        TermModel.findOneAndUpdate(
            ...
            if(err) {
                ...
                res.json({"result": req.body.term + " Updated"});
            }
    ...
    app.post('/api/delete', function(req,res) {
        TermModel.findOneAndRemove({'term': {'$regex' : '^' + req.body.term, '$options' : 'i'}},
            ...
            if(err) {
                ...
                res.json({"result": found_term.term + " Deleted"});
            }
